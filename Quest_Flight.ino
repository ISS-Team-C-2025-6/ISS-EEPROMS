...
public:
  ...
  char* to_string(int eeprom_num, int page_num) {
    ...
    if (success == READ_SUCCESS) {
      ...
      int len = strlen(buffer);
      snprintf(buffer + len, buffer_size - len, "page read failure!");
    }
    return buffer;
  }
};
...
constexpr uint8_t EEPROM_ADDRESS[] = { 0b1010001, 0b1010101, 0b1010110, 0b1010011 };  //0x51, 0x55, 0x56, 0x53  ;        0x50 is likely the FRAM ;
...
void Flying() {
  ...
  //
  // for (int i=0;i<4;i++){
  //     write_whole_eeprom(PATTERN[i],EEPROM_ADDRESS[i]);
  // }
  while (1) {
    ...
    if ((millis() - TimeEvent1) > TimeEvent1_time) {
      ...
      Serial.println("begin read loop");
       for (int i = 1; i < 4; i++) {
         read_whole_eeprom(PATTERN[i], EEPROM_ADDRESS[i]);
         Serial.print(i + 1);
         Serial.println(" eeprom finished");
      //  }

       Serial.println("finished reading everything, logging data------------------------------");
      databufferLength = 0;
      databuffer[0] = '\0';
      appendToBuffer("HELLO\r\n");
      // Serial.print("len(databufferLength)=");
...
void write_whole_eeprom(byte data, uint8_t ADD) {
  for (int j = 0; j < 32000; j++) {
    if (j % 320 == 0) Serial.println("5 page");
    write(0, j, data, ADD);
  }
}
void read_whole_eeprom(char data, uint8_t ADD) {
  char time_buffer[16];
  snprintf(time_buffer, 16, "%lu", (unsigned long)rtc.now().unixtime());  //avoid calloc every time stamp
  databufferLength = 0;
  databuffer[0] = '\0';  // critical because nophoto30K uses strlen/strcat
  appendToBuffer(time_buffer);
  appendToBuffer(" - ");
